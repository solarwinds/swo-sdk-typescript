 ...
 * Get outage statuses
 */
export function demGetWebsiteOutageStatuses(
  client: SwoCore,
  ...
  PageIterator<
    Result<
      operations.GetWebsiteOutageStatusesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetWebsiteOutageStatusesRequest$outboundSchema.parse(value),
  ...
  const path = options?.[URL_OVERRIDE]
    ? ""
    : pathToFunc("/v1/dem/websites/{entityId}/outageStatuses")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetWebsiteOutageStatusesResponse$inboundSchema, {
      key: "Result",
    }),
