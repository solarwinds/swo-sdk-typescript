 ...
 */
export function demGetWebsite(
  client: SwoCore,
  request: operations.GetWebsiteRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetWebsiteResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetWebsiteRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/dem/websites/{entityId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetWebsiteResponse$inboundSchema),
