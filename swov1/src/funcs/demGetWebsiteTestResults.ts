 ...
 * Get test results
 */
export function demGetWebsiteTestResults(
  client: SwoCore,
  ...
  PageIterator<
    Result<
      operations.GetWebsiteTestResultsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetWebsiteTestResultsRequest$outboundSchema.parse(value),
  ...
  const path = options?.[URL_OVERRIDE]
    ? ""
    : pathToFunc("/v1/dem/websites/{entityId}/testResults")(pathParams);
  ...
  const query = options?.[URL_OVERRIDE]
    ...
    : encodeFormQuery({
      ...
      "maxResponse": payload.maxResponse,
      "minResponse": payload.minResponse,
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetWebsiteTestResultsResponse$inboundSchema, {
      key: "Result",
    }),
