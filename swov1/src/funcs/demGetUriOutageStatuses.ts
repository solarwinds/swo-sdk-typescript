 ...
 * Get outage statuses
 */
export function demGetUriOutageStatuses(
  client: SwoCore,
  ...
  PageIterator<
    Result<
      operations.GetUriOutageStatusesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetUriOutageStatusesRequest$outboundSchema.parse(value),
  ...
  const path = options?.[URL_OVERRIDE]
    ? ""
    : pathToFunc("/v1/dem/uris/{entityId}/outageStatuses")(pathParams);
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetUriOutageStatusesResponse$inboundSchema, {
      key: "Result",
    }),
