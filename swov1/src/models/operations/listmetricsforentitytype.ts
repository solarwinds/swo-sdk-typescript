/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod";
import { safeParse } from "../../lib/schemas.js";
import { Result as SafeParseResult } from "../../types/fp.js";
import * as components from "../components/index.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";

export type ListMetricsForEntityTypeRequest = {
  type: string;
  /**
   * Timestamp in ISO 8601 format in UTC timezone: yyyy-MM-ddTHH:mm:ssZ
   */
  startTime?: Date | undefined;
  /**
   * Timestamp in ISO 8601 format in UTC timezone: yyyy-MM-ddTHH:mm:ssZ
   */
  endTime?: Date | undefined;
};

/**
 * The request has succeeded.
 */
export type ListMetricsForEntityTypeResponse = {
  type: string;
  /**
   * A list of metrics and their metadata. An empty list indicates no metrics matched the given parameters.
   */
  metrics: Array<components.CommonMetricInfo>;
};

/** @internal */
export const ListMetricsForEntityTypeRequest$inboundSchema: z.ZodType<
  ListMetricsForEntityTypeRequest,
  z.ZodTypeDef,
  unknown
> = z.object({
  type: z.string(),
  startTime: z.string().datetime({ offset: true }).transform(v => new Date(v))
    .optional(),
  endTime: z.string().datetime({ offset: true }).transform(v => new Date(v))
    .optional(),
});

/** @internal */
export type ListMetricsForEntityTypeRequest$Outbound = {
  type: string;
  startTime?: string | undefined;
  endTime?: string | undefined;
};

/** @internal */
export const ListMetricsForEntityTypeRequest$outboundSchema: z.ZodType<
  ListMetricsForEntityTypeRequest$Outbound,
  z.ZodTypeDef,
  ListMetricsForEntityTypeRequest
> = z.object({
  type: z.string(),
  startTime: z.date().transform(v => v.toISOString()).optional(),
  endTime: z.date().transform(v => v.toISOString()).optional(),
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace ListMetricsForEntityTypeRequest$ {
  /** @deprecated use `ListMetricsForEntityTypeRequest$inboundSchema` instead. */
  export const inboundSchema = ListMetricsForEntityTypeRequest$inboundSchema;
  /** @deprecated use `ListMetricsForEntityTypeRequest$outboundSchema` instead. */
  export const outboundSchema = ListMetricsForEntityTypeRequest$outboundSchema;
  /** @deprecated use `ListMetricsForEntityTypeRequest$Outbound` instead. */
  export type Outbound = ListMetricsForEntityTypeRequest$Outbound;
}

export function listMetricsForEntityTypeRequestToJSON(
  listMetricsForEntityTypeRequest: ListMetricsForEntityTypeRequest,
): string {
  return JSON.stringify(
    ListMetricsForEntityTypeRequest$outboundSchema.parse(
      listMetricsForEntityTypeRequest,
    ),
  );
}

export function listMetricsForEntityTypeRequestFromJSON(
  jsonString: string,
): SafeParseResult<ListMetricsForEntityTypeRequest, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ListMetricsForEntityTypeRequest$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ListMetricsForEntityTypeRequest' from JSON`,
  );
}

/** @internal */
export const ListMetricsForEntityTypeResponse$inboundSchema: z.ZodType<
  ListMetricsForEntityTypeResponse,
  z.ZodTypeDef,
  unknown
> = z.object({
  type: z.string(),
  metrics: z.array(components.CommonMetricInfo$inboundSchema),
});

/** @internal */
export type ListMetricsForEntityTypeResponse$Outbound = {
  type: string;
  metrics: Array<components.CommonMetricInfo$Outbound>;
};

/** @internal */
export const ListMetricsForEntityTypeResponse$outboundSchema: z.ZodType<
  ListMetricsForEntityTypeResponse$Outbound,
  z.ZodTypeDef,
  ListMetricsForEntityTypeResponse
> = z.object({
  type: z.string(),
  metrics: z.array(components.CommonMetricInfo$outboundSchema),
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace ListMetricsForEntityTypeResponse$ {
  /** @deprecated use `ListMetricsForEntityTypeResponse$inboundSchema` instead. */
  export const inboundSchema = ListMetricsForEntityTypeResponse$inboundSchema;
  /** @deprecated use `ListMetricsForEntityTypeResponse$outboundSchema` instead. */
  export const outboundSchema = ListMetricsForEntityTypeResponse$outboundSchema;
  /** @deprecated use `ListMetricsForEntityTypeResponse$Outbound` instead. */
  export type Outbound = ListMetricsForEntityTypeResponse$Outbound;
}

export function listMetricsForEntityTypeResponseToJSON(
  listMetricsForEntityTypeResponse: ListMetricsForEntityTypeResponse,
): string {
  return JSON.stringify(
    ListMetricsForEntityTypeResponse$outboundSchema.parse(
      listMetricsForEntityTypeResponse,
    ),
  );
}

export function listMetricsForEntityTypeResponseFromJSON(
  jsonString: string,
): SafeParseResult<ListMetricsForEntityTypeResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ListMetricsForEntityTypeResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ListMetricsForEntityTypeResponse' from JSON`,
  );
}
