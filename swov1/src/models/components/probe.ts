/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod";
import { safeParse } from "../../lib/schemas.js";
import { ClosedEnum } from "../../types/enums.js";
import { Result as SafeParseResult } from "../../types/fp.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";

/**
 * Cloud platform hosting the probe.
 */
export const Platform = {
  Aws: "AWS",
  Azure: "AZURE",
  GoogleCloud: "GOOGLE_CLOUD",
} as const;
/**
 * Cloud platform hosting the probe.
 */
export type Platform = ClosedEnum<typeof Platform>;

/**
 * Coordinates of the probe.
 */
export type Coordinates = {
  /**
   * Latitude of the location.
   */
  latitude: number;
  /**
   * Longitude of the location.
   */
  longitude: number;
};

export type Probe = {
  /**
   * Unique identifier of the probe.
   */
  id: string;
  /**
   * Name of the probe.
   */
  name: string;
  /**
   * True if the probe is active.
   */
  active: boolean;
  /**
   * Cloud platform hosting the probe.
   */
  platform: Platform;
  /**
   * Region where the probe is located.
   */
  region: string;
  /**
   * ISO 3166-1 alpha-2 code of the country where the probe is located.
   */
  country: string;
  /**
   * City where the probe is located.
   */
  city: string;
  /**
   * Coordinates of the probe.
   */
  coordinates: Coordinates;
  /**
   * IPv4 addresses of the probe.
   */
  ipv4Addresses: Array<string>;
  /**
   * IPv6 addresses of the probe. Not all probes support IPv6.
   */
  ipv6Addresses?: Array<string> | undefined;
};

/** @internal */
export const Platform$inboundSchema: z.ZodNativeEnum<typeof Platform> = z
  .nativeEnum(Platform);

/** @internal */
export const Platform$outboundSchema: z.ZodNativeEnum<typeof Platform> =
  Platform$inboundSchema;

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace Platform$ {
  /** @deprecated use `Platform$inboundSchema` instead. */
  export const inboundSchema = Platform$inboundSchema;
  /** @deprecated use `Platform$outboundSchema` instead. */
  export const outboundSchema = Platform$outboundSchema;
}

/** @internal */
export const Coordinates$inboundSchema: z.ZodType<
  Coordinates,
  z.ZodTypeDef,
  unknown
> = z.object({
  latitude: z.number(),
  longitude: z.number(),
});

/** @internal */
export type Coordinates$Outbound = {
  latitude: number;
  longitude: number;
};

/** @internal */
export const Coordinates$outboundSchema: z.ZodType<
  Coordinates$Outbound,
  z.ZodTypeDef,
  Coordinates
> = z.object({
  latitude: z.number(),
  longitude: z.number(),
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace Coordinates$ {
  /** @deprecated use `Coordinates$inboundSchema` instead. */
  export const inboundSchema = Coordinates$inboundSchema;
  /** @deprecated use `Coordinates$outboundSchema` instead. */
  export const outboundSchema = Coordinates$outboundSchema;
  /** @deprecated use `Coordinates$Outbound` instead. */
  export type Outbound = Coordinates$Outbound;
}

export function coordinatesToJSON(coordinates: Coordinates): string {
  return JSON.stringify(Coordinates$outboundSchema.parse(coordinates));
}

export function coordinatesFromJSON(
  jsonString: string,
): SafeParseResult<Coordinates, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => Coordinates$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'Coordinates' from JSON`,
  );
}

/** @internal */
export const Probe$inboundSchema: z.ZodType<Probe, z.ZodTypeDef, unknown> = z
  .object({
    id: z.string(),
    name: z.string(),
    active: z.boolean(),
    platform: Platform$inboundSchema,
    region: z.string(),
    country: z.string(),
    city: z.string(),
    coordinates: z.lazy(() => Coordinates$inboundSchema),
    ipv4Addresses: z.array(z.string()),
    ipv6Addresses: z.array(z.string()).optional(),
  });

/** @internal */
export type Probe$Outbound = {
  id: string;
  name: string;
  active: boolean;
  platform: string;
  region: string;
  country: string;
  city: string;
  coordinates: Coordinates$Outbound;
  ipv4Addresses: Array<string>;
  ipv6Addresses?: Array<string> | undefined;
};

/** @internal */
export const Probe$outboundSchema: z.ZodType<
  Probe$Outbound,
  z.ZodTypeDef,
  Probe
> = z.object({
  id: z.string(),
  name: z.string(),
  active: z.boolean(),
  platform: Platform$outboundSchema,
  region: z.string(),
  country: z.string(),
  city: z.string(),
  coordinates: z.lazy(() => Coordinates$outboundSchema),
  ipv4Addresses: z.array(z.string()),
  ipv6Addresses: z.array(z.string()).optional(),
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace Probe$ {
  /** @deprecated use `Probe$inboundSchema` instead. */
  export const inboundSchema = Probe$inboundSchema;
  /** @deprecated use `Probe$outboundSchema` instead. */
  export const outboundSchema = Probe$outboundSchema;
  /** @deprecated use `Probe$Outbound` instead. */
  export type Outbound = Probe$Outbound;
}

export function probeToJSON(probe: Probe): string {
  return JSON.stringify(Probe$outboundSchema.parse(probe));
}

export function probeFromJSON(
  jsonString: string,
): SafeParseResult<Probe, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => Probe$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'Probe' from JSON`,
  );
}
